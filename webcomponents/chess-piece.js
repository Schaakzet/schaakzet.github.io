...
!(function () {
  ...
  customElements.define(
    window.CHESS.__WC_CHESS_PIECE__,
    class extends window.CHESS.ChessBaseSquarePieceElement {
      // ======================================================== <chess-piece>.observedAttributes
      static get observedAttributes() {
        return [window.CHESS.__WC_ATTRIBUTE_IS__]; // listen to is attribute
      }
      ...
      // ======================================================== <chess-piece>.is
      get is() {
        return this.getAttribute(window.CHESS.__WC_ATTRIBUTE_IS__);
      }
      set is(value) {
        this.setAttribute(window.CHESS.__WC_ATTRIBUTE_IS__, value);
      }
      ...
      // ======================================================== <chess-piece>.fen_at
      get fen_at() {
        return window.CHESS.convertFEN(this.is) + this.at;
      }
      // ======================================================== <chess-piece>.color
      get color() {
        const indexStreepje = this.is.indexOf(window.CHESS.__PIECE_SEPARATOR__);
        return this.is.slice(0, indexStreepje);
      }
      // ======================================================== <chess-piece>.isWhite
      get isWhite() {
        return this.color == window.CHESS.__PLAYER_WHITE__;
      }
      // ======================================================== <chess-piece>.isBlack
      get isBlack() {
        return this.color == window.CHESS.__PLAYER_BLACK__;
      }
      // ======================================================== <chess-piece>.isPawn
      get isPawn() {
        return this.isPiece(window.CHESS.__PIECE_PAWN__);
      }
      // ======================================================== <chess-piece>.isWhitePawn
      get isWhitePawn() {
        return this.is == window.CHESS.__PLAYER_WHITE__ + window.CHESS.__PIECE_SEPARATOR__ + window.CHESS.__PIECE_PAWN__;
      }
      // ======================================================== <chess-piece>.isWhitePawn
      get isBlackPawn() {
        return this.is == window.CHESS.__PLAYER_BLACK__ + window.CHESS.__PIECE_SEPARATOR__ + window.CHESS.__PIECE_PAWN__;
      }
      // ======================================================== <chess-piece>.isPiece
      isPiece(name) {
      ...
      // ======================================================== <chess-piece>.isRook
      get isRook() {
        return this.isPiece(window.CHESS.__PIECE_ROOK__);
      }
      // ======================================================== <chess-piece>.isKnight
      get isKnight() {
        return this.isPiece(window.CHESS.__PIECE_KNIGHT__);
      }
      // ======================================================== <chess-piece>.isBishop
      get isBishop() {
        return this.isPiece(window.CHESS.__PIECE_BISHOP__);
      }
      // ======================================================== <chess-piece>.isKing
      get isKing() {
        return this.isPiece(window.CHESS.__PIECE_KING__);
      }
      // ======================================================== <chess-piece>.isQueen
      get isQueen() {
        return this.isPiece(window.CHESS.__PIECE_QUEEN__);
      }
      ...
      get pieceMoves() {
        let _moves = [];
        if (this.isKnight) {
          _moves = window.CHESS.__HORSEMOVES__;
        } else if (this.isBishop) {
          _moves = window.CHESS.__BISHOPMOVES__;
        } else if (this.isRook) {
          _moves = window.CHESS.__ROOKMOVES__;
        } else if (this.isQueen) {
          _moves = window.CHESS.__QUEENMOVES__;
        } else if (this.isKing) {
          _moves = window.CHESS.__KINGMOVES__;
        } else if (this.isPawn) {
          if (this.isWhitePawn) {
            _moves = [[[0, 1]]];
      ...
      potentialMoves() {
            ...
            if (_squareName) {
              ...
              if (_squareElement.piece) {
                if (this.isPawn) {
                  ...
                  _squareElement.highlight(window.CHESS.__PROTECT_PIECE__); // TODO:: move to .defendedBy() method
                  _squareElement.defendedBy(this);
                } else {
                  // not a pawn
                  // Als het een andere kleur heeft,window.CHESS.__ATTACK_PIECE__, potentialMove!
                  // attackedby="Nb6,Qf3"
                  _squareElement.highlight(window.CHESS.__ATTACK_PIECE__); // TODO:: move to .attackedBy() method
                ...
                _squareElement.highlight(window.CHESS.__EMPTY_SQUARE__);
                _potentialMovesArray.push(_squareName);
                if (!this.isPawn) {
                  _squareElement.defendedBy(this);
                }
        ...
        if (this.pieceName === window.CHESS.__PIECE_WHITE_PAWN__) {
          pawnAttack(window.CHESS.__PLAYER_BLACK__, -1, 1);
          pawnAttack(window.CHESS.__PLAYER_BLACK__, 1, 1);
        } else if (this.pieceName === window.CHESS.__PIECE_BLACK_PAWN__) {
          pawnAttack(window.CHESS.__PLAYER_WHITE__, 1, -1);
          pawnAttack(window.CHESS.__PLAYER_WHITE__, -1, -1);
        }
      ...
      potentialKingMoves() {
        ...
        if (isKing) {
          ...
          const longWhiteRook = $chessboard.getPiece(window.CHESS.__SQUARE_BOTTOM_LEFT__);
          const shortWhiteRook = $chessboard.getPiece(window.CHESS.__SQUARE_BOTTOM_RIGHT__);
          const longBlackRook = $chessboard.getPiece(window.CHESS.__SQUARE_TOP_LEFT__);
          const shortBlackRook = $chessboard.getPiece(window.CHESS.__SQUARE_TOP_RIGHT__);
          ...
          const longWhiteCastling = isCastling(window.CHESS.__FEN_WHITE_QUEEN__, longWhiteRook, "d1");
          const shortWhiteCastling = isCastling(window.CHESS.__FEN_WHITE_KING__, shortWhiteRook, "f1");
          const longBlackCastling = isCastling(window.CHESS.__FEN_BLACK_QUEEN__, longBlackRook, "d8");
          const shortBlackCastling = isCastling(window.CHESS.__FEN_BLACK_KING__, shortBlackRook, "f8");

          function castlingInterrupt(color, offset) {
            // True: Castling interrupted
            let kingPosition = $chessboard.getSquare(color == window.CHESS.__PLAYER_WHITE__ ? window.CHESS.__SQUARE_WHITE_KING_START__ : window.CHESS.__SQUARE_BLACK_KING_START__);

            let checkInterrupt = (i) => {
              let squareName = kingPosition.translate(i, 0);
              let squareElement = $chessboard.getSquare(squareName);
              if (squareElement.isDefendedBy(window.CHESS.otherPlayer(color)) || kingPosition.attackers.length) return true;
            };
          ...
          function checkCastlingInterrupt(offset, squareName) {
            if (!castlingInterrupt(playerColor, offset)) {
              ...
              square.squareElement(squareName).highlight(window.CHESS.__EMPTY_SQUARE__);
              _potentialMovesArray.push(squareName);
            } else console.log("Castling not possible", squareName);
          }

          if (playerColor == window.CHESS.__PLAYER_WHITE__) {
          ...
          const allowedMoves = _potentialMovesArray.filter((squareName) => {
            ...
            const ultimateDefenders = defender_square.defenders.filter((defender_FENat) => {
              let myFENpos = window.CHESS.convertFEN(this.is) + this.at;
          ...
          // NEW DEFENDED ARRAY
          const kingPosition = window.CHESS.convertFEN(this.is) + this.at;
          for (const squarename of falseMoves) {
            ...
            square.setAttribute(window.CHESS.__WC_ATTRIBUTE_DEFENDEDBY__, newDefendedArray);
            square.style.border = "";
            // NEW ATTACKED ARRAY
            if (square.getAttribute(window.CHESS.__WC_ATTRIBUTE_ATTACKEDBY__)) {
              const attackedby = square.attackers;
              const newAttackedArray = attackedby.filter((attacker) => {
                return attacker != kingPosition;
              });
              square.setAttribute(window.CHESS.__WC_ATTRIBUTE_ATTACKEDBY__, newAttackedArray);
              square.style.border = "";
            }
          }
        }
      ...
      animateTo(destinationSquare) {
        ...
        return this.animate([{ transform: `translateX(0px) translateY(0px)` }, { transform: `translateX(${destLeft - left}px) translateY(${destTop - top}px)` }], {
          duration: window.CHESS.__MOVEPIECE_ANIMATION_DURATION__,
          iterations: 1,
        ...
        this.moves.forEach((to) => {
          ...
          if (showboardsIn) {
            ...
            testboard = showboardsIn.appendChild(
              window.CHESS.createBoardElement({
                props: {
                  id: window.CHESS.__TESTBOARD_FOR_MOVES__,
